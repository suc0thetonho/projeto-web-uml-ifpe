// Define o formato que o backend espera receber
interface ILoginRequest {
    email: string;
    senha: string;
}

// Define o formato que o backend deve devolver em caso de sucesso
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formLogin.addEventListener('submit', async (evento: Event) => {
        // Previne o recarregamento padrão da página
        evento.preventDefault();

        const dadosLogin: ILoginRequest = {
        ...
        try {
            ...
            //  O  backend precisa criar exatamente essa rota '/api/auth/login'
            ...
            const dados: ILoginResponse = await resposta.json();
